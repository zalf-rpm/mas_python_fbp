    ...
    async def run(self):
        ...
        while True:
            ...
            try:
                ...
                if not await self.write_out("out", _data_ip(geoparquet_bytes), automatic_chunking=True):
                    logger.info("%s process finished", self.name)
                    return
                _ = await self.write_out("raster", _data_ip(raster_bytes), automatic_chunking=True)
